...
def load_config() -> dict:
    ...
    # this daemon (Claude token, Google OAuth, --zai-key,
    # --device-secret-key) is deliberately kept OUT of this file already --
    # see the module-level comments on _zai_api_key/_device_secret_key. This
    # only guards against a stale key surviving from a manual edit or an old
...
# Device write-auth key (SmallTV's own "Access" card, Update tab). Sent as
# X-Secret-Key on every push (usage/calendar/weather) so a device with a key
# set rejects pushes from anyone else, including a daemon pointed at the
# wrong device. Set once from --device-secret-key/CLAWDMETER_SECRET_KEY in
# main(); intentionally NOT persisted to ~/.clawdmeter-daemon.json (same
# treatment as CLAUDE_CODE_OAUTH_TOKEN -- a credential, not a UI preference).
_device_secret_key = ""

# z.ai's own API key (for the quota endpoint, unrelated to _device_secret_key
# above). Set once from --zai-key/CLAWDMETER_ZAI_KEY in main(); same
# not-persisted-to-disk treatment as _device_secret_key -- a credential, not
# a UI preference.
_zai_api_key = ""


def _push_headers() -> dict:
    return {"X-Secret-Key": _device_secret_key} if _device_secret_key else {}
...
def _codex_rpc_call() -> dict | None:
    ...
    against a real account: this spins up a local subprocess and reads the
    already-cached rate-limit state the CLI's auth layer maintains -- it
    does NOT send a prompt/fire a completion, round-trips in ~1-2s. This
    replaced an earlier design that grepped the most recently modified
    ~/.codex/sessions/*/*/*/*.jsonl rollout file for a `rate_limits` block
    written by real usage; the RPC is simpler (no file globbing, no risk of
    reading a stale/wrong rollout) and was confirmed exactly as cheap, so
    the old passive/active-ping split (--codex-ping) no longer applies --
    there's only one mode now. `codex login` must already be done on
...
def push_loop(stop: threading.Event, targets_fn, interval: float) -> None:
    ...
    failure never blocks the others. Only trusted targets get the secret-key
    header -- an mDNS-discovered device hasn't been vetted as actually being
    the user's own.
    """
...
def main() -> None:
    ...
    ap.add_argument("--device-secret-key", default=None,
                    help="the SmallTV's own write-auth key (device's Update tab -> Access card, "
                         "if set) -- sent as X-Secret-Key on every push (usage/calendar/weather/zai/"
                         "codex) so the device rejects pushes from anyone but this daemon. Also "
                         "settable via CLAWDMETER_SECRET_KEY. Leave unset if the device has no key set.")
    ...
    global _device_secret_key, _zai_api_key
    _device_secret_key = args.device_secret_key or os.environ.get("CLAWDMETER_SECRET_KEY", "")
    ...
    # _zai_api_key is NOT stored in cfg -- same not-persisted-to-disk
    # treatment as _device_secret_key, it's a credential not a UI preference.
    if args.codex:
