...
# Set once when Google answers 401/403 for a calendar, cleared on the next success, so
# the cause is visible in the log without repeating every poll.
_calendar_auth_logged = False
...
def poll_calendar(token: str, override_ids: list[str] | None) -> tuple[dict | None, bool]:
    ...
    for calendar_id, color in calendars:
        try:
            ...
            continue
        if resp.status_code in (401, 403):
            # Log it once per streak: a 403 here is almost always "Calendar API has
            # not been used in project ..." (API not enabled, or the key belongs to a
            # different project). Swallowing it silently sent people hunting the share
            # step instead. The retry path below re-fetches the token for a real 401.
            if not _calendar_auth_logged:
                log(f"Calendar API HTTP {resp.status_code} ({calendar_id}): "
                    f"{resp.text[:200]}")
                _calendar_auth_logged = True
            auth_failed = True
            continue
        if resp.status_code >= 400:
...
# ---------------------------------------------------------------------------
# Album: photos on the device, converted here rather than there.
#
# The SmallTV has no image decoder and no room for one, and 240x240 is 115,200
# bytes -- more than four times its free heap. So the device never decodes and
# never buffers a frame: this end resizes, centre-crops and converts to RGB565,
# and the firmware streams the bytes it receives straight to the panel.
#
# Cadence is driven BY THE DEVICE, not by a timer here. Every push is answered
# with {"next": <seconds until the album page is on screen again>}, so this loop
# sleeps exactly that long. No polling, and carousel drift corrects itself on
# every push. next == -1 means the page is not ticked in the device's carousel.
ALBUM_W = 240
ALBUM_H = 240
ALBUM_BYTES = ALBUM_W * ALBUM_H * 2
ALBUM_DEFAULT_DIR = Path.home() / "Pictures" / "claudedaemon"
ALBUM_CACHE_DIR = Path.home() / ".clawdmeter-album-cache"
ALBUM_EXTS = {".jpg", ".jpeg", ".png", ".bmp", ".gif", ".webp"}
ALBUM_IDLE_SEC = 300.0      # page not in rotation / no pictures: check back in 5 min
ALBUM_RETRY_SEC = 60.0      # device unreachable or gave no usable "next"

_album_warned: set = set()


def _album_warn_once(key: str, msg: str) -> None:
    """One line per distinct problem, not one per push -- this loop can run
    every few seconds and would otherwise bury the log."""
    if key not in _album_warned:
        _album_warned.add(key)
        log(msg)


def album_files(folder: Path) -> list:
    """Every picture under folder, recursively, in a stable order so the
    remembered index means the same thing across restarts."""
    try:
        return sorted(p for p in folder.rglob("*")
                      if p.is_file() and p.suffix.lower() in ALBUM_EXTS)
    except OSError as e:
        _album_warn_once("scan", f"Album: cannot read {folder}: {e}")
        return []


def album_frame(path: Path) -> bytes | None:
    """One picture as a 115,200-byte big-endian RGB565 frame.

    Cached on disk keyed by path+mtime+size, so a folder that has not changed
    converts nothing at all after the first pass. Pillow is already a dependency
    (the tray icon), and the pixel loop is pure Python on purpose -- numpy is not
    a dependency and 57,600 iterations once per picture is a few tens of ms.
    """
    try:
        st = path.stat()
    except OSError:
    ...
    try:
        ...
        _album_warn_once("pillow", "Album: Pillow not installed -- pip install -r requirements.txt")
        return None
    try:
    ...
    out = bytearray(ALBUM_BYTES)
    for i in range(ALBUM_W * ALBUM_H):
        r = rgb[3 * i]; g = rgb[3 * i + 1]; b = rgb[3 * i + 2]
        v = ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3)
        out[2 * i] = v >> 8                     # big-endian: the firmware bswaps per row
        out[2 * i + 1] = v & 0xFF
    data = bytes(out)
    try:
        ...
        tmp.replace(hit)                        # atomic: never leave a torn cache entry
    except OSError as e:
        _album_warn_once("cache", f"Album: cannot write cache {ALBUM_CACHE_DIR}: {e}")
    return data


def album_push_loop(stop: threading.Event, targets_fn, folder: Path, shuffle: bool) -> None:
    """Push one picture per device rotation, remembering where we got to."""
    log(f"Album: {folder} (order: {'shuffled' if shuffle else 'by name'})")
    if not folder.is_dir():
        log(f"Album: {folder} does not exist yet -- create it and drop pictures in, "
            f"no restart needed")
    delay = 5.0                                  # let the first usage push land first
    while not stop.is_set():
        stop.wait(delay)
        if stop.is_set():
            break
        delay = ALBUM_RETRY_SEC

        files = album_files(folder)
        urls = targets_fn()
        if not files or not urls:
            if not files:
                _album_warn_once("empty", f"Album: no pictures in {folder}")
            delay = ALBUM_IDLE_SEC
            continue
        _album_warned.discard("empty")

        cfg = load_config()
        idx = int(cfg.get("album_index", 0))
        if shuffle:
            idx = random.randrange(len(files))
        path = files[idx % len(files)]

        data = album_frame(path)
        if data is None:                         # unreadable: step over it next time
            cfg["album_index"] = (idx + 1) % len(files)
            save_config(cfg)
            delay = 1.0
            continue
        ...
        sent = False
        for url in urls:
            album_url = _resolve_url_for_push(url.replace("/api/usage", "/api/album"))
            try:
                # Multipart, not a raw body: the firmware reads this through the
                # streaming upload handler, the only path that does not buffer
                # the whole 115 KB in RAM.
                r = httpx.post(album_url,
                               files={"frame": ("frame.565", data, "application/octet-stream")},
                               timeout=30.0, headers=_push_headers())
                if r.status_code >= 400:
                    log(f"Album push {album_url} HTTP {r.status_code}")
                    continue
                sent = True
                try:
                    n = r.json().get("next")
                ...
                if isinstance(n, (int, float)):
                    # Several devices: wake for whichever needs a frame soonest.
                    if n >= 0 and (nxt is None or n < nxt):
                        nxt = float(n)
            except httpx.HTTPError as e:
                log(f"Album push {album_url} failed: {e}")
        ...
        if nxt is not None:
            ...
            # Every device answered -1: the Pictures page is not ticked in the
            # carousel, so there is nothing to feed until the owner ticks it.
            _album_warn_once("untick", "Album: device has the Pictures page unticked "
                                       "(web UI -> Display -> Carousel) - idling")
            delay = ALBUM_IDLE_SEC
...
ANTIGRAVITY_PORT_WAIT_S   = 90   # `agy` can stall ~58s before listening on a
                                 # DNS-less boot; must exceed that comfortably
ANTIGRAVITY_STATUS_WINDOW_S = 20  # budget for real configs, AFTER ports exist
...
def _antigravity_wait_for_status(pid: int, port_deadline: float,
                                 status_window_s: float = 20.0) -> dict | None:
    ...
    Port discovery gets its OWN, longer deadline. Confirmed live on a host
    that reboots on a schedule: when clawdmeter starts ~55s after boot and
    DNS is not answering yet, `agy` stalls ~58s BEFORE its language server
    starts listening at all. Sharing one 20s budget between "wait for the
    ports to exist" and "wait for real model configs" meant that stall ate
    the entire window and surfaced as the misleading "is `agy` authenticated
    on this machine?" -- three times in two days, always ~2 minutes after a
    reboot. The status budget now only starts counting once the ports are
    actually there."""
    last_status: dict | None = None
    ports = _antigravity_find_ports(pid, port_deadline)
    if not ports:
        return None
    # The status budget is measured from HERE, not from spawn, which is the
    # whole point of the split: a slow listen must not eat the time we need to
    # wait out the startup race. Two explicit parameters rather than one
    # deadline plus a max() -- an earlier version took a `deadline` and then
    # silently extended it, so a caller's shorter deadline was ignored while
    # still looking honoured.
    deadline = time.time() + status_window_s
    while time.time() < deadline:
        for port in ports:
            result = _antigravity_rpc_call_once(port)
            if not isinstance(result, dict) or "userStatus" not in result:
                continue
            last_status = result
            status = result.get("userStatus") or {}
            cascade = status.get("cascadeModelConfigData") or {}
            if cascade.get("clientModelConfigs"):
                return result
        time.sleep(1.0)
    return last_status
...
def poll_antigravity(_retry: bool = True) -> dict:
    ...
    if not agy_bin:
        ...
        return {"ok": False}
    # Make `agy` choose its file-based token store outright. agy 1.1.28 picks a
    # store by three rules, in order: an SSH session (keyed on SSH_CONNECTION)
    # -> file, never touching the keyring; a keyring timeout recorded in the
    # last hour -> file; otherwise keyring first. A service has no SSH_* vars,
    # so it always took the third path and met Secret Service on every poll.
    # What that costs depends on the box and BOTH were observed live: with no
    # default collection the read fails in under a millisecond (harmless), but
    # against a `login` collection that exists and is LOCKED -- the normal state
    # on a headless boot where nobody types the login password -- it blocks on
    # an unlock prompt no one can answer for 5s + 10s, once an hour, forever.
    # The plaintext token file is what agy falls through to anyway.
    # This is ALSO why the same command works when you ssh in and fails as a
    # unit: ssh sets SSH_CONNECTION. It was never a permissions difference.
    agy_env = dict(os.environ)
    agy_env.setdefault("SSH_CONNECTION", "127.0.0.1 0 127.0.0.1 22")
    try:
        p = subprocess.Popen(
    ...
    result = _antigravity_wait_for_status(
        p.pid, time.time() + ANTIGRAVITY_PORT_WAIT_S,
        status_window_s=ANTIGRAVITY_STATUS_WINDOW_S)
...
def main() -> None:
    ...
    ap.add_argument("--calendar", action="store_true",
                    help="enable the Google Calendar feature (needs a service-account key -- "
                         "see README.md -- or --calendar-auth run once for OAuth)")
    ...
    # Album (optional feature, off by default). Pictures live HERE, not on the
    # device: this end resizes and converts, the device just paints what arrives.
    ap.add_argument("--album", nargs="?", const=str(ALBUM_DEFAULT_DIR), default=None,
                    metavar="DIR",
                    help=f"enable the Pictures page, feeding photos from DIR "
                         f"(default: {ALBUM_DEFAULT_DIR}). Also tick Pictures in the "
                         f"device's carousel settings.")
    ap.add_argument("--no-album", action="store_true",
                    help="disable the album feature (overrides a previously remembered --album)")
    ap.add_argument("--album-shuffle", action="store_true",
                    help="pick pictures at random instead of in filename order")
    ...
    if args.album is not None:
        cfg["album_enabled"] = True
        cfg["album_dir"] = args.album
    if args.no_album:
        cfg["album_enabled"] = False
    if args.album_shuffle:
        cfg["album_shuffle"] = True
    if args.zai:
    ...
    # Album: independent of the primary Transport, same reasoning as z.ai --
    # and unlike every other feature it has no poll interval of its own, because
    # the device's push reply tells it when the next frame is wanted.
    if cfg.get("album_enabled"):
        album_targets = tuple(_static_push_urls(cfg))
        if album_targets:
            album_dir = Path(cfg.get("album_dir") or ALBUM_DEFAULT_DIR).expanduser()
            album_stop = threading.Event()
            threading.Thread(target=album_push_loop,
                             args=(album_stop, lambda: album_targets, album_dir,
                                   bool(cfg.get("album_shuffle"))),
                             daemon=True).start()
        else:
            log("Album enabled but no --push-to host configured - nothing to push to")
