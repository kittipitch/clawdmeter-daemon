...
REM The daemon uses PEP 604 annotations: on 3.9 it dies at import with an opaque
REM TypeError. Refuse early rather than build a venv that cannot run it.
%PYCMD% -c "import sys; sys.exit(0 if sys.version_info >= (3,10) else 1)" || ( echo Python 3.10+ required -- install it from python.org and tick "Add python.exe to PATH". & goto :fail )
