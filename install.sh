...
# The daemon uses PEP 604 annotations, so 3.9 dies at import with a bewildering
# "TypeError: unsupported operand type(s) for |". Stock macOS still ships 3.9, and a
# .venv built from it is reused silently on every later run -- refuse it up front.
if ! "$BASE_PY" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 10) else 1)'; then
    echo "Python 3.10+ required; $BASE_PY is $("$BASE_PY" -c 'import sys; print("%d.%d" % sys.version_info[:2])')."
    echo "macOS:  PYTHON=\$(brew --prefix)/bin/python3 ./install.sh"
    echo "Linux:  install a newer python3, then PYTHON=/path/to/python3.12 ./install.sh"
    exit 1
fi
if [ -x "$VPY" ] && ! "$VPY" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 10) else 1)'; then
    echo "The existing .venv was built with Python $("$VPY" -c 'import sys; print("%d.%d" % sys.version_info[:2])') -- too old. Removing it."
    rm -rf "$VENV"
fi
