...
- **Secrets go in the environment, not the unit file.** Put
  `CLAUDE_CODE_OAUTH_TOKEN=...` (and any of `CLAWDMETER_ZAI_KEY`,
  `CLAWDMETER_SECRET_KEY`, etc. you use) in a separate file, e.g.
...
`summary`/`start` are `null` when there's no upcoming event. Field names are
...
browser — don't use it over SSH.) Credentials land in `~/.codex/auth.json`
either way, so once logged in on one machine, copying that file to another
(`scp ~/.codex/auth.json <host>:~/.codex/auth.json`) is also an option,
same pattern as the Calendar token below — not independently verified
here, but it's a plain JSON credential file, not a live session handle.

**"no rate limits returned" even though `codex login status` says you're
logged in?** Two things to check, confirmed live on a real deployment:

- A stderr line like `Codex's Linux sandbox uses bubblewrap and needs
  access to create user namespaces` is **cosmetic noise on modern
  `codex-cli`** (confirmed via `RUST_LOG` tracing) — it does *not* block
  `account/rateLimits/read`, which never actually needs the sandbox. Don't
  chase kernel/AppArmor settings (e.g.
  `kernel.apparmor_restrict_unprivileged_userns`) to fix this; it's a red
  herring for this specific RPC call.
- The real cause found here was an **old `codex-cli` build silently
  swallowing the RPC error** — upgrading (`npm install -g
  @openai/codex@latest`, or `sudo npm install -g @openai/codex@latest` if
  it was installed system-wide) surfaced the actual problem underneath:
  a `refresh_token_reused` 401, meaning the locally-stored OAuth refresh
  token had already been consumed server-side (this can happen if a
  previous `codex` process completes a token exchange but gets killed
  before writing the new token back to `~/.codex/auth.json` — e.g. during
  manual debugging with a timeout/kill). Fix is the same either way:
  `codex login --device-auth` again to get a fresh token.
...
--device-secret-key KEY  the SmallTV's own write-auth key (its web UI's
                    Update tab -> Access card), if you've set one there.
                    Sent as X-Secret-Key on every push (usage/calendar/
                    weather/zai/codex/antigravity) so the device only
                    accepts pushes from this daemon. Env:
                    CLAWDMETER_SECRET_KEY. Leave unset if the device has no
                    key set (its default -- open).
...
### Device write-auth (secret key)

Newer `smalltv-mod` builds let you set a secret key in the device's own web
UI (Update tab -> Access card) that gates writes, **including this daemon's
own usage/calendar/weather/zai pushes** -- not just browser-facing config
changes. If you've set one there, this daemon needs the matching key too
(`--device-secret-key`/`CLAWDMETER_SECRET_KEY`), or every push gets
rejected and the device's screen goes stale. **Update the daemon with the
key before setting it on the device**, not after, to avoid that gap.
Leave both unset if you haven't configured a key on the device -- it's
fully open by default, same as ever.
